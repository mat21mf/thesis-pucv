# ### deje kmn0001 para grupos 02 al 00 por error en var08
# ### corregir en GenerarBatchBigKmeans.sh OK
# ### Pendiente
# ### - concatenar totss
# ###
# find -type f -name 'ProcesoBigKmeans_kmn00*var08*' \
#   | sort -t'_' -k3,3 | xargs ls -tr # | grep -i --color 'kmn[0]\{3\}[1]'
  ...
  bash -c \"\${EscribirConcatenacionObjetos01}\" \
    ...
    | sed -r '2,\$s/^  /, /' \
    | sed -r '1i\save(' \
    | sed -r '\$a\, file=\"kmn_var08_objetos.RData\" )'
  "
  ...
  bash -c "${EscribirConcatenacionObjetos03}" \
      > ConcatenarBigKmeans_objetos_var08.R
